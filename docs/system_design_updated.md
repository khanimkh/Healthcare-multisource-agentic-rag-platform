...
Amazon RDS PostgreSQL stores application metadata such as uploaded documents, ingestion jobs, processing status, users, chat history, and evaluation results. Structured datasets (CSV, Excel, Parquet) are stored in Amazon S3, registered in AWS Glue Data Catalog, and queried using Amazon Athena.
...
AWS Glue Data Catalog stores metadata for structured datasets only, including table schemas, column types, S3 locations, partitions, and file formats. Application metadata is stored in Amazon RDS PostgreSQL, while chunk metadata is stored in Amazon OpenSearch.
...
The platform separates metadata into three layers:

#### Amazon RDS PostgreSQL (Application Metadata)
...
glue_table
error_message
users, chat history, ingestion jobs and audit logs
```

#### Amazon OpenSearch (Chunk Metadata)
...
Chunks are inserted using bulk indexing to improve ingestion performance.
...
Glue does not store embeddings, document chunks, processing status, or users.

### Production Asynchronous Ingestion
...
Upload original file to Amazon S3
	->
Create document record in Amazon RDS
	->
Publish ingestion job (Amazon SQS)
	->
Return response immediately

Background Worker (ECS/Lambda)
...
Generate Bedrock embeddings
	->
Create bulk indexing actions
	->
Bulk index into Amazon OpenSearch
	->
Register structured datasets in AWS Glue
	->
Update document status in Amazon RDS
```
...
4. Optionally reranks retrieved passages.
5. Generates a source-grounded response using Amazon Bedrock.
...
1. Extracts relevant entities.
2. Searches Amazon Neptune for related nodes and relationships.
3. Retrieves supporting document evidence.
4. Combines graph knowledge with document retrieval.
5. Generates a relationship-aware answer using Amazon Bedrock.
